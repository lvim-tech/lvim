-- Custom floating diagnostic popup for LVIM IDE.
--
...
--
---@module "modules.base.configs.languages.lsp.diagnostics"
        ...
        if wrap_at and width >= wrap_at then
            ...
            if vim.tbl_isempty(line_widths) then
                for _, line in ipairs(contents) do
                    height = height + math.ceil(vim.fn.strdisplaywidth(line) / wrap_at)
                end
            else
                for i = 1, #contents do
...
local function open_floating_preview(contents, syntax)
    ...
    local width, height = make_floating_popup_size(contents, { max_width = 130 })
...
---@return integer|nil floating_bufnr
---@return integer|nil floating_winnr
