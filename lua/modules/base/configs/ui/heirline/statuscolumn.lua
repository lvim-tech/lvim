...
M.get_statuscolumn = function()
    ...
    local function get_extmarks(bufnr, lnum, filter_func)
        ...
        local ok, extmarks = pcall(vim.api.nvim_buf_get_extmarks, bufnr, -1, { lnum - 1, 0 }, { lnum - 1, -1 },
            { details = true })
    ...
    static.handlers.MiniDiffPreview = function(_, args)
        if not mini_ok or not mini then return end
        ...
        if type(get_buf_data) ~= "function" then return end
        local data = get_buf_data(buf)
        if not data or type(data.hunks) ~= "table" or #data.hunks == 0 then return end

        local found = nil
        for _, h in ipairs(data.hunks) do
            local from = math.max(h.buf_start, 1)
            local to = (h.buf_count and h.buf_count > 0)
                and (h.buf_start + h.buf_count - 1)
                or from
        ...
        local preview_lines, line_meta = {}, {}
        table.insert(preview_lines, string.format("%s hunk — buf:%d..%d | ref:%d..%d", found.type, found.buf_start,
            (found.buf_start or 0) + (found.buf_count or 0) - 1, found.ref_start,
        ...
        pcall(function() vim.bo[buf_preview].filetype = "diff" end)
        pcall(function() vim.bo[buf_preview].modifiable = false end)
        ...
        for i, meta in ipairs(line_meta) do
            ...
            local hl_group = ({
                add = md_hl.over_add,
                delete = md_hl.over_delete,
                change_ref = md_hl.over_change,
                change_buf = md_hl.over_change_buf,
            })[meta.kind]
            if hl_group then
                pcall(vim.api.nvim_buf_set_extmark, buf_preview, ns, ln - 1, 0, { hl_group = hl_group, hl_eol = true })
            end
        end
    ...
    local line_numbers = {
        ...
        provider = function(self)
            ...
            local spaces_needed = max_len - #str
            return string.rep(" ", spaces_needed) .. str
        end,
        hl = function(self)
