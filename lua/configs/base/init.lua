    ...
    vim.api.nvim_create_autocmd("ColorScheme", {
        ...
        callback = function()
            ...
            _G.LVIM.colors = {
                ...
                cyan     = cyan,
                purple   = purple,
                blue_bh  = blue_bh,   blue_bl  = blue_bl,
                green_bh = green_bh,  green_bl = green_bl,
                orange_bh= orange_bh, orange_bl= orange_bl,
                red_bh   = red_bh,    red_bl   = red_bl,
                cyan_bh  = cyan_bh,   cyan_bl  = cyan_bl,
                purple_bh= purple_bh, purple_bl= purple_bl,
...
configs["base_events"] = function()
    ...
    vim.api.nvim_create_autocmd("FileType", {
        pattern  = {
            "NeogitStatus", "Outline", "calendar",
            "dapui_breakpoints", "dapui_scopes", "dapui_stacks", "dapui_watches",
            "git", "netrw", "org", "toggleterm",
            "fyler", "Fyler", "neo-tree", "time-machine-list",
        },
        callback = function()
...
-- Merge base and user file-type tables into _G.LVIM.file_types and remove
-- the default LSP keymaps injected by Neovim so LVIM can define its own.
configs["base_languages"] = function()
    ...
    -- Merge base language definitions with any user overrides.
    _G.LVIM.file_types = funcs.merge(base_file_types, user_file_types)
end
...
configs["base_commands"] = function()
    ...
    -- Initialise GxPlus (universal gx replacement) and bind it to gx.
